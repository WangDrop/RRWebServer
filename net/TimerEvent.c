...
static void resetTimer(int timerfd, struct timeval expire)
{
	...
	timerfd_settime(timerfd, TFD_TIMER_ABSTIME, &new, &old); //通过文件描述符可读时间，事件来进行超时通知
}
...
static void onTimeout(struct EventLoop* loop, void* arg)
{
	...
	while(!empty(queue))//定时事件到期的时候开始依次的处理时间
	{
		latest = top(queue);
...
void timerInit(struct TimerEvent* tevent, struct EventLoop* loop)
{
	...
	tevent->event = newEvent(timerfd, EV_READ, 
						(readCallback)onTimeout, NULL, tevent, loop); //将这个tevent作为参数
...
//add一个新的timer的时候会使用新的timer的值比较选择是否替换值
...
//删除掉一个定时器之后也同时需要根据情况来修改定时器事件使用resetTimer来进行修改
...
//新建一个Timer，注意这里这个事件是相对时间而不是绝对时间
